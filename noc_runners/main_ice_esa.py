...
def main():
    ...
    pentad_climatology = obs_module.read_climatology(
        cci_climatology, lat_south, lat_north, lon_west, lon_east
    )
    ...
    for i in range(len(year_list)):
        ...
        ncfilename = (
            str(output_directory) + str(current_year) + "_pentads_kriged_esa.nc"
        )
        ...
        for j in range(len(month_list)):
            ...
            for pentad_idx, pentad_day in enumerate(range(3, 29, 5)):
                ...
                if pentad_idx < 5:
                    day_df = obs_df[
                        (pentad_day - 2 <= obs_df["dy"])
                        & (obs_df["dy"] < pentad_day + 3)
                        & (obs_df["mo"] == current_month)
                    ]
                elif pentad_idx == 5:
                    day_df = obs_df[
                        (pentad_day - 2 <= obs_df["dy"])
        ...
        print(krig_anom.shape)
        print(krig_uncert.shape)
