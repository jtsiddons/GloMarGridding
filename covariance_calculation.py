...
import covariance_variogram as cov_var
...
def calculate_pca_and_eof(cci_mask, chosen_ipc=True):
    ...
    if chosen_ipc:
        ...
        print('Number of PCs has not been specified by the user, set to variance ratio over 70%')
...
def reconstruct_covariance_from_eofs(sst_flat_with_nans, lat_1d, lon_1d, chosen_ipc=True):
    ...
    EOF_recons = np.ma.masked_values(np.reshape(EOF_recons, (ipc, len(lat), len(lon)), order='C'), -999.)
    print('Reconstructed', EOF_recons.shape)

    plt.imshow(EOF_recons[2,:,:], origin='lower', interpolation='nearest', aspect='auto')
...
def plot_empirical_reconstructed_covariance(cci_mask, dat_from_eofs):
    ...
    fig.suptitle('Original CCI SST anomaly covariance (1), covariance using reconstructed data from 3 EOFs (2), and difference (1-2)')
...
def covariance_from_variogram(sst_flat_with_nans,lat_1d, lon_1d,variance):
    ...
    covariance_variogram = cov_var.variogram(distance_matrix, variance)
