...
def main(argv):
    ...
    for i in range(len(year_list)):
        ...
        for j in range(len(month_list)):
            ...
            if processing == 'pentad': 
                for pentad_idx, pentad_day in enumerate(range(3,29,5)):
                    print(pentad_idx)
                    print(pentad_day)
                    if pentad_idx < 5:
                        day_df = obs_df[(pentad_day - 2 <= obs_df['dy']) & (obs_df['dy'] < pentad_day+3)]
                    elif pentad_idx == 5:
                        day_df = obs_df[(pentad_day - 2 <= obs_df['dy'])]
                    
                    timestep = int(i+(current_month-1)*6)
                    print('---------')
                    print(day_df['dy'])
                    print('----------')
                    ...
                    esa_climatology = climatology['climatology'].values[timestep,:,:]
    ...
    for i in range(len(year_list)):
        current_year = year_list[i]
        climatology = obs_module.read_pentad_climatology(cci_climatology)
        print(climatology)
        ...
        for j in range(len(month_list)):
            ...
            if processing == 'pentad': 
                for pentad in range(1,7):
                    print(current_year, current_month, pentad)
                    ...
                    esa_climatology = obs_module.read_climatology(cci_climatology,DOY, time_type='pentad')
                    print(esa_climatology)
                    ...
                    cropped_clim = cropped_clim.mean(dim='time')
                    print(cropped_clim.values)
                    ...
                    print(coarse_cropped_clim)
                    coarse_cropped_clim = coarse_cropped_clim.mean(dim='time')
                    print(coarse_cropped_clim)
                    """
                    esa_climatology = climatology['climatology'].values[timestep,:,:]
                    #add climatology value and calculate the SST anomaly
            ...
            #for daily processing
                ...
                for day in range(1,month_range+1,1): #unique_days)):
                    ...
                    esa_climatology = obs_module.read_climatology(cci_climatology,DOY, time_type='daily')
                    cropped_clim = esa_climatology.sel(lat=slice(lat_south,lat_north), lon=slice(lon_west,lon_east))
                    print(cropped_clim)
                    #climatology comes at 0.05 lat/lon resolution
                    #ouput product comes at 1 degree, needs coarsening by 20
    ...
    print('Processing for chosen date range successfully finished')
    print('%s to %s') % (year_start, year_stop)
