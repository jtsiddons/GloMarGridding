...
from nonstationary_cov import cube_covariance_nonstationary_stich as cube_cov_stich
...
profiler.enable()
hfix_cpd = True
stich_hfix = cube_cov_stich.CovarianceCube_PreStichedLocalEstimates(Lx, Ly,
                                                                    theta,
                                                                    sigma,
                                                                    v=v,
...
no_hfix_cpd = False
stich_no_hfix = cube_cov_stich.CovarianceCube_PreStichedLocalEstimates(Lx, Ly,
                                                                       theta,
                                                                       sigma,
                                                                       v=v,
                                                                       delta_x_method=delta_x_method,
...
xy = 150
cor_with_point_cube0 = stich_hfix.remap_one_point_2_map(stich_hfix.cor_ns[xy,:].copy(),
                                                        cube_name='correlation',
                                                        cube_unit='1')
cor_with_point_cube1 = stich_no_hfix.remap_one_point_2_map(stich_no_hfix.cor_ns[xy,:].copy(),
                                                           cube_name='correlation',
                                                           cube_unit='1')
...
pd_check_coord = AuxCoord(int(stich.check_positive_definite), 
...
det_coord = AuxCoord(stich.cov_det, long_name = 'covariance_determinant')
eig_coord = AuxCoord(stich.cov_eig[-1], long_name = 'smallest_eigenvalue')
pd_check_coord = AuxCoord(int(stich.check_positive_definite), long_name = 'positive_semidefinite_check_enabled')
# Define the iris cube
cov_cube = Cube(stich_cov, dim_coords_and_dims = [(dim_row, 0), (dim_col, 1)])
cor_cube = Cube(stich_cor, dim_coords_and_dims = [(dim_row, 0), (dim_col, 1)])
cov_cube.data = cov_cube.data.astype(np.float32)
