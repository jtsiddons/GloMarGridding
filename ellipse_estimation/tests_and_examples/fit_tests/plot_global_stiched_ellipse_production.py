...
def reverse_mask_from_compress_1D(data_cube_2D,
                                  compressed_1D_vector,
                                  fill_value=0.0,
                                  dtype=np.float32):
    compressed_counter = 0
    print(data_cube_2D.data.mask.shape)
    err_msg = 'Unexpected number of dimensions; len(shape)='+str(len(data_cube_2D.data.mask.shape))
...
def lat_mask(old_cube, lat_max=85.0):
    if lat_max < 90.0:
        ...
        _, lat_mesh = np.meshgrid(new_cube.coord('longitude').points, 
...
def draw_ellipse(latlon_point, figlabel, mm, london_in_the_middle=True):
    ...
    cube_fu = cube_io_10.iris_load_cube_plus(ncfiles,
                                             var_name=varname,
    ...
    cor_basepath = '/noc/mpoc/surface_data/ESA_CCI5deg_month_extra/ANOMALY/SpatialScales/'
    cor_basepath = cor_basepath+'locally_build_covariances/'
    cor_file = cor_basepath+'covariance_'+str(mm).zfill(2)+'_v_eq_1p5_sst_clipped.nc'
    cor_cube = iris.load_cube(cor_file, 'correlation')
    map_table_file = cor_basepath+'row_lookup_'+str(mm).zfill(2)+'_v_eq_1p5_sst_without_psd_check.csv'
    ...
    corr_contours2 = np.array([-0.9, -0.7, -0.5, -0.3, -0.1, 0.1, 0.3, 0.5, 0.7, 0.9])
    s_size = 100
    ...
    search_bol = (map_table.lat == latlon_point[1]) & (map_table.lon == latlon_point[0])
    matching_rows = map_table[search_bol]
    assert_msg = 'There should be only one unique matching row, nrows='+str(len(matching_rows))
    ...
    print(np.min(R_cube.data), np.min(R_cube_x), 
          np.max(R_cube.data), np.max(R_cube_x))

    # Get ellipse parameters
    lon_grid, lat_grid = np.meshgrid(R_cube.coord('longitude').points,
                                     R_cube.coord('latitude').points)
    latlon_pairs = np.deg2rad(np.column_stack([lat_grid.reshape(-1),
                                               lon_grid.reshape(-1)]))
    ...
    xys = EllipseModel().predict_xy(np.linspace(-np.pi, np.pi, 360),
                                        params=params_dist)
    print(xys)
    ...
    proj4_cartopy = ccrs.TransverseMercator(central_longitude=gdf_lon_mean,
                                            central_latitude=gdf_lat_mean)
    df_0p5 = pd.DataFrame({'northing': xys[:, 1], 'easting': xys[:, 0]})
    points_0p5 = df_0p5.apply(lambda row: Point([row.easting, row.northing]), axis=1)
    ...
    distance_ii[distance_ii>np.pi] = distance_ii[distance_ii>np.pi]-2.0*np.pi
    distance_ii = distance_ii*average_cos

    zonal_displacement = distance_ii * cube_cov._RADIUS_OF_EARTH / cube_cov._KM2M
    meridonal_displacement = distance_jj * cube_cov._RADIUS_OF_EARTH / cube_cov._KM2M
    ...
    def _corr_func_dist(x_dist, y_dist):
        return cube_cov.c_ij_anistropic_rotated(v, 
                                                1.0,
                                                x_dist, 
                                                y_dist,
                                                float(Lx_dist.data),
    ...
    predicted_corr_dist = corr_func_dist(zonal_displacement_cube.data,
                                         meridonal_displacement_cube.data)
    ...
    predicted_corr_dist_cube2.data = np.ma.masked_where(R_cube.data.mask, 
                                                        predicted_corr_dist_cube2.data)
    ...
    cf = iplt.contourf(R_cube,
                       levels=corr_contours2,
                       cmap=corr_cmap2,
                       extend='both')
    cl = iplt.contour(predicted_corr_dist_cube2,
                      levels=corr_contours2,
                      colors='k')
    ax.scatter(latlon[0]-360, latlon[1],
               s=s_size, 
               color='black', 
               transform=ccrs.Geodetic())
    title = 'Knitted SST corr with x='+str(latlon_point[0])+' y='+str(latlon_point[1])+' \n'
    ...
    fig_name = fig_path+figlabel+'_'+str(mm).zfill(2)+'_stiched_corr_with_ellipse_correlation_contour.png'
    plt.savefig(fig_name)
    plt.clf() 
...
def main():
    ...
    for mm in mms:
        for lonlat_pt, figlabel, london_in_the_middle in zip(lonlat_points, figlabels, london_in_the_middles):
