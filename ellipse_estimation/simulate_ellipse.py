...
def fill_cube_with_uniform_parms(cube_template, sdev, sigma_parms):
    """
...
class EllipseSimulation_UniformParms(_EllipseSimulation):
    def __init__(self, v, cube_template, sdev_uniform, sigma_parms_uniform):
        parm_cubes = fill_cube_with_uniform_parms(
            cube_template, sdev_uniform, sigma_parms_uniform
        )
...
def chisq_test_using_likelihood_ratios_4_covariance(sigma_hat, sigma_actual, n):
    ...
    assert sigma_hat.shape == sigma_actual.shape, (
        "sigma_hat shape does not agree with sigma_actual.shape"
    )
    sigma_hat_unbiased = (
        (n / (n - 1)) * sigma_hat
...
def regmean_simulated_ellipse_vs_known_regmean(
    ...
    Lx_hat, Ly_hat, theta_hat_deg = average_LxLyTheta(
        Lx_estimated, Ly_estimated, theta_estimated
    )
    theta_hat = np.deg2rad(theta_hat_deg)
    W, p_val = simulated_ellipse_vs_known_regmean(
        [Lx_hat, Ly_hat, theta_hat],
