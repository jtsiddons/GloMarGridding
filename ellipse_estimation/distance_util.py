...
def haversine2(lon1, lat1, lon2, lat2):
    ...
    This is an alternative to sklearn version, and can run faster
...
def scalar_cube_great_circle_distance(
    ...
    delta_x_method = "Spherical_COS_Law":
    uses COS(C) = COS(A)COS(B)

    delta_x_method = "Met_Office":
    Suspected flat (cylindrical) Earthers in Exeter!
    delta_x = 6400km x delta_lon (in radians)
    ...
    Spherical law of cosines pose a lot of challenge in use of a Earth
    coordinate system that is based on lat lon, you can say you walk in
    some direction for delta_lat x R_earth and turn 90 degree towards
    to the other point, but you are actually not following lines
    ...
    COS(GREAT_CIRCLE_DIST_IN_RADIANS) = COS(A) COS(B)
    provided angle between A and B = pi/2
    ...
    A less "bad" approximation based on the Met Office approach is to
