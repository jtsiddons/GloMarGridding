...
def as65_to_hs93_effective_range(L):
    ...
    This converts L definition used in AS65/RW05 to the one used in HS93
    and GSTAT (for 10 >= v >= 0.5)
    """
    return L * 2.0 / np.sqrt(2)


# In HadCRUT5 paper, they mentioned 1300km scale
# The 1300km in that paper is based on the conventions used
...
def ESA_landfrac(
    ...
    Have no lakes, but lakes are included in HadCRUT5 weight files and ERA5.
    However, for temperature kriging, it is not clear if lakes should be treated
    as land or even open sea, so defaults using for sea or land
    or even interpolating from nearby grid point (mostly land) be valid.
    ERA LSAT kriging scales exclude lakes because lakes are labeled as
    water pixels.
...
#     '''
#     In HadCURT5 analysis weight file, any pixels that have a shred
...
def infill_scales(
    ...
    fudge_long_scales : bool
        Should Lx, Ly, theta, sdev be modified if convergence_qc values
        that are flagged
