...
ncpaths = ['/noc/mpoc/surface_data/ESA_CCI5deg_month_extra/ANOMALY/SpatialScales/locally_build_covariances/',
...
for ncpath, obsname in zip(ncpaths, obsnames):
    ...
    for mm in mms:
        ncfile = ncpath+'covariance_'+str(mm).zfill(2)+'_v_eq_1p5_'+obsname+'_clipped.nc'
        csvfile = ncpath+'row_lookup_'+str(mm).zfill(2)+'_v_eq_1p5_'+obsname+'_without_psd_check.csv'
        outfile = ncpath+'covariance_'+str(mm).zfill(2)+'_v_eq_1p5_'+obsname+'_clipped_0_360.nc'
        # Lfile = ncpath+'covariance_'+str(mm).zfill(2)+'_v_eq_1p5_'+obsname+'_clipped_L_0_360.npy'
        latlon_lookup = pd.read_csv(csvfile)
        latlon_lookup['lon_alt'] = latlon_lookup['lon'].apply(lambda lon: lon+360 if lon < 0 else lon)
        ...
        for new_row, old_row in zip(latlon_lookup_alt['new_row_num'], latlon_lookup_alt['row_num']):
            H[new_row, old_row] = 1
        print(H)
        assert np.all(np.sum(H, axis=0) == 1), 'Your mapping matrix is not working! Only a single 1 per axis=0'
        assert np.all(np.sum(H, axis=1) == 1), 'Your mapping matrix is not working! Only a single 1 per axis=1'
