...
def time_check(xa_time, yyyy0=1850, yyyyf=2023):
    ...
    return np.logical_and(
        yyyyf >= datetime64_to_yyyy(xa_time),
        datetime64_to_yyyy(xa_time) >= yyyy0,
    )


def lsat_error_covariance_maker(
    ensemble_mem, savez_compressed=True, yyyy0=1850, yyyyf=2023
):
    """
    ...
    if savez_compressed:
        out_error_covariance_npy = (
            outpath + "lsat_error_covariance_" + str(ensemble_mem) + ".npz"
        )
    else:
        out_error_covariance_npy = (
            outpath + "lsat_error_covariance_" + str(ensemble_mem) + ".npy"
        )
    ...
    xa_sampling_rnd_sigma_2 = xa_sampling_rnd_sigma_2[
        timecheck2(xa_sampling_rnd_sigma_2["time"].values)
    ]
    print(xa_sampling_rnd_sigma_2)
    times = xa_sampling_rnd_sigma_2.time.values
    n_times = len(times)
    xy_size = len(xa_sampling_rnd_sigma_2.lat.values) * len(
        xa_sampling_rnd_sigma_2.lon.values
    )
...
def sst_error_covariance_maker(yyyy, mm, savez_compressed=True):
    """
    Create SST error covariance matrices
    ...
    coastal_pixels = np.logical_and(
        ~clim_cube_sst.data.mask, ~clim_cube_lsat.data.mask
    )
    ...
    terrain_cat = (
        coastal_pixels * tac_map["coast"]
        + land_pixels * tac_map["land"]
        + sea_pixels * tac_map["sea"]
    )
    terrain_cat_flatten = terrain_cat.flatten()
    terrain_cat_flatten_series = [
        cat_map[val] for val in terrain_cat_flatten.tolist()
    ]
    terrain_cat_flatten_series = pd.Series(
        terrain_cat_flatten_series, dtype="category"
    )
    #
    # Now into error covariances
    outpath = nc_path + "error_covariances/"
    out_latlon_table_feather = (
        outpath + "latlon_" + str(yyyy) + "_" + str(mm).zfill(2) + ".feather"
    )
    if savez_compressed:
        out_error_covariance_npy = (
            outpath
            + "sst_error_covariance_"
            + str(yyyy)
            + "_"
            + str(mm).zfill(2)
            + ".npz"
    ...
    latlon_lookup_table = pd.DataFrame(
        {"index": idx, "latitude": yy_flatten, "longitude": xx_flatten}
    )
    ...
    for row_idx in range(nrows):
        ...
        if nan_check:
            ...
            assertion_msg += (
                "old_val = " + str(old_val) + " ; forcing sigma2 to 0 ."
            )
            # assert ~valid_nan_check, assertion_msg
            print(assertion_msg)
            warnings.warn(assertion_msg, UserWarning)
