...
def laloux_clip(
    ...
    """
    Based on the following 2 papers:
    - https://doi.org/10.1142/S0219024900000255 (Laloux et al 2000, L2000)
    - https://doi.org/10.1016/j.physrep.2016.10.005 (Section 7.2.2)

    The goal is to estimate the largest eigenvalue that one will get from
    covariance or correlation matrices that are generated by random uncorrelated
    matrices.

    Eq 0.3 in L2000 says eigenvalues of covariances generated by uncorrelated
    random vectors have a min and max of:

    lambda_min = sigma**2 (1 + Q - 2 SQRT(Q))
    lambda_max = sigma**2 (1 + Q + 2 SQRT(Q))

    in which:

    Q = num_of_features / length_of_each_feature

    One can make a simple Monte-Carlo simulation to check that.

    Hence lambda_max can be thought of a floor for non-noise EOFs,
    ...
    Any eigenvalues smaller than lambda_max (including negative ones) are
    ...
    num_grid_pts : int
        Number of spatial grid points for covariance, this should
        usually be the shape of the covariance matrix
...
def explained_variance_clip(
    ...
    Wilks, D. (2006). Statistical Methods in the Atmospheric Sciences (2nd ed.).
    Elsevier.
